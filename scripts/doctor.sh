...
#   lock_unreadable               actas lock could not be read (diagnostic_failure/messaging, registration)
#   lock_empty                    actas lock present but empty / torn write (condition/messaging, registration)
#   lock_liveness_unknown         lock owner liveness could not be determined (diagnostic_failure/messaging, registration)
...
# Per-scope registration observation (core only): lock is
# none|unreadable|empty|alive|stale|unknown, watcher is
# running|stale-pidfile|none (empty = not applicable, null).
_doctor_reg_add() {
    ...
    while IFS=$'\t' read -r team agent; do
      ...
      case "${_own_r%%$'\t'*}" in
        unreadable)
          ...
          _redact_project "$project"
          _doctor_warn lock_unreadable diagnostic_failure messaging "$project" "$type" \
            registration "$team" "$agent" "" -- \
            "[$_REDACT_OUT] lock could not be read: $dteam/$dagent (not reported as absent)"
          _doctor_reg_add "$project" "$type" "$team" "$agent" "unreadable" ""
          continue
          ;;
        absent)
      ...
      if [ -z "$owner" ]; then
        ...
        _redact_project "$project"
        _doctor_warn lock_empty condition messaging "$project" "$type" \
          registration "$team" "$agent" "" -- \
          "[$_REDACT_OUT] lock file is present but empty: $dteam/$dagent (torn write; not reported as free)"
        _doctor_reg_add "$project" "$type" "$team" "$agent" "empty" ""
        continue
      fi
      _any_owner=1
      ...
      case "$_alive_rc" in
        ...
        *) alive_word="unknown"
           _redact_project "$project"
           _doctor_warn lock_liveness_unknown diagnostic_failure messaging "$project" "$type" \
             registration "$team" "$agent" "" -- \
             "[$_REDACT_OUT] lock owner liveness could not be determined: $dteam/$dagent (owner=$(_redact_owner "$owner")); not treating it as stale" ;;
      esac
      ...
      case "$alive_word" in
        alive) _doctor_reg_add "$project" "$type" "$team" "$agent" "alive" "$watcher_val" ;;
        STALE) _doctor_reg_add "$project" "$type" "$team" "$agent" "stale" "$watcher_val" ;;
        *)     _doctor_reg_add "$project" "$type" "$team" "$agent" "unknown" "$watcher_val" ;;
      esac
