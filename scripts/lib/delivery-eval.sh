  ...
  if [ "$plug_has_status_override" -eq 0 ]; then
    ...
    if [ -f "$hf" ]; then
      ...
      if [ "$valid" = "1" ]; then
        ...
        # SKILL_DIR is contractually set by both sourcing contexts; when it is
        # absent there is no install path to match, so leave has_ss/has_st at 0
        # (fail-closed into "unrecognized") rather than letting instr() match
        # an empty needle.
        local skill_dir_sql=""
        [ -n "${SKILL_DIR:-}" ] && skill_dir_sql="$(printf '%s' "$SKILL_DIR" | sed "s/'/''/g")"
        if [ -n "$skill_dir_sql" ]; then
